...
func NewBlockVoting(bc *core.BlockChain, chainConfig *params.ChainConfig, txpool *core.TxPool, mux *event.TypeMux, db ethdb.Database, accountMgr *accounts.Manager, isSynchronised bool) *BlockVoting {
	bv := &BlockVoting{
		bc:     bc,
		cc:     chainConfig,
		txpool: txpool,
...
func (bv *BlockVoting) Start(client *rpc.Client, strat BlockMakerStrategy, ks *keystore.KeyStore, voteAcct, makerAcct accounts.Account) error {
	bv.ks = ks
			...
			case event, ok := <-sub.Chan():
				...
				case core.ChainHeadEvent: // got a new header, reset pending state
					...
					if bv.synced {
						...
						if tx, err := bv.vote(number, e.Block.Hash()); err == nil {
							log.Debug("Voted for %s on height %d in tx %s", e.Block.Hash().Hex(), number, tx.Hex())
						} else {
							log.Error("Unable to vote: %v", err)
						}
					}
				...
				case CreateBlock:
					...
					if err == nil && e.Hash != nil {
						...
						log.Error("Unable to create block: %v", err)
					}
...
func (bv *BlockVoting) vote(height *big.Int, hash common.Hash) (common.Hash, error) {
	...
	log.Info("vote for %s on height %d", hash.Hex(), height)
